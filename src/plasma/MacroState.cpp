...
MacroState::MacroState(FILE *& InputDeck)
{
	...
	if (_plasma->uses_full_PIC())
	{
		...
		_parameterization = std::unique_ptr<MacroParameterization>(new MacroParameterizationFullPICtoHistogram(initialization));
	}
	else	
	{
		...
		//_parameterization = std::unique_ptr<MacroParameterization>(new MacroParameterizationPUREHaar(initialization, vte));
	}
	_parameterization->Initialize(*_micro_state);
}

MacroState::MacroState(FILE *& InputDeck, int number_of_microsteps)
{
	MacroParameterizationFromFile initialization = MacroParameterizationFromFile(InputDeck, number_of_microsteps);
	_plasma = initialization.get_plasma();
	_micro_state = std::unique_ptr<State>(new State(initialization));
	_simulation_time = _micro_state->get_simulation_time();
	_macro_iteration = std::make_shared<int>(0);
